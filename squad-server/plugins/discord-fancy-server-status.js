...
export default class DiscordFancyServerStatus extends DiscordIntervalUpdatedMessage {
    ...
    buildCurrentLayerImage = (currentLayer) => {
        const currentLayerUrl = `https://squad.gamepedia.com/Special:FilePath/${this.layerNameToSquadWikiImageName(
            currentLayer
        )}.jpg`;
    ...
    buildNextLayerThumbnail = (nextLayer) => {
        const nextLayerUrl = `https://squad.gamepedia.com/Special:FilePath/${this.layerNameToSquadWikiImageName(
            nextLayer
        )}.jpg`;
    ...
    buildMessage = (server) => {
        ...
        return {
            embed: {
                ...(this.displayCurrentLayerImage && server.layerHistory[0] && server.layerHistory[0].layer
                    ? this.buildCurrentLayerImage(server.layerHistory[0].layer)
                    : {}),
                ...(this.displayNextLayerThumbnail && server.nextLayer
                    ? this.buildNextLayerThumbnail(server.nextLayer)
                    : {}),
